...
func doMap(
	...
	for i, _ := range reduceFiles {
		file, err := os.OpenFile(reduceName(jobName, mapTaskNumber, i), os.O_WRONLY|os.O_CREATE, 0666)
		if err != nil {
			log.Fatal("check: ", err)
		}
		reduceFiles[i] = file
	...
	input, err := os.OpenFile(inFile, os.O_RDONLY, 0666)
	if err != nil {
		log.Fatal("check: ", err)
	}
	defer input.Close()
	...
	for _, kv := range kvs {
		...
		enc := json.NewEncoder(reduceFiles[reduceNum])
		err := enc.Encode(&kv)
		if err != nil {
			log.Fatal(err)
		}
